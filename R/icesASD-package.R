#' @docType package
#'
#' @name icesASD-package
#'
#' @aliases icesASD
#'
#' @title Stock Advice and Scenarios Database Web Services
#'
#' @description
#' R interface to access the web services of the ICES Stock Advice
#' and Scenarios database.
#'
#' @details
#' \emph{Get dataset:}
#' \tabular{ll}{
#'   \code{\link{get_advice_view_info}}             \tab Get Advice Record\cr
#'   \code{\link{get_catch_scenario_table}}             \tab Get Advice Catch Scenario Table\cr
#'  \code{\link{get_catch_scenario_notes}}             \tab Get Advice Catch Scenario Table Footnotes
#' 
#' }
#'
#' @author Colin Millar, Luca Lamoni, Carlos Pinto, Laura Petre, David Miller and Neil Maginis.
#'
#' @references
#' ICES Advice and Scenarios database: \url{http://sg.ices.dk}.
#'
#' ICES Advice and Scenarios database web services:
#' \url{http://sg.ices.dk/webservices.aspx}.

NA
